...
def test_send_text_invalid_lang():
    ...
    assert answer == [{'Error': "Either of the languages may not be available, ('enf', 'es')"}]

def test_send_text_invalid_type():
    answer = translate_text("hello", 23, ['es'])
    assert answer == [{"Error": "Either of the languages may not be available, (23, 'es')"}]

def test_send_text_invalid_target():
    answer = translate_text("Hello", "en", ['es','frr',12])
    assert answer == [{"text":"Hola.", "lang":"es"}, {"Error": "Either of the languages may not be available, ('en', 'frr')"}, {"Error": "Either of the languages may not be available, ('en', 12)"}]
